...
class MainPanel extends JPanel implements KeyListener, Runnable, Common {
    ...
    private void gameRender() {
        ...
        // 勇者の位置にスポットライトをセット
        spotlight.setSpot(hero.getPx()+offsetX+Chara.CS/2, hero.getPy()+offsetY+Chara.CS/2, 128);
        Rectangle2D screen = new Rectangle2D.Double(0, 0, WIDTH, HEIGHT);
        Area mask = new Area(screen);  // 画面全体を覆うマスク
        mask.subtract(new Area(spotlight.getSpot()));  // スポットライト部分のマスクを取り去る
        
        Graphics2D g2 = (Graphics2D)dbg;
        Color shieldColor = new Color(0.0f, 0.0f, 0.0f);  // Color(0.0f, 0.0f, 0.0f, 0.5f)とすると半透明
        g2.setColor(shieldColor);
        g2.fill(mask);  // マスク部分を黒で塗りつぶす
